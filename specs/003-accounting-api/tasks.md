...
- [x] T046 Execute every command in `specs/003-accounting-api/quickstart.md` against a CE Sail installation and record expected status codes and ledger results — full run recorded in `docs/qa/ce-api-quickstart-20260910.md`; all steps matched expected status codes, and an incomplete Prerequisites ability list found during the run was corrected in `quickstart.md`.
...
- [x] T054 Add focused coverage for concurrent transitions, rate-limit metadata, failed workflow retries, token/resource isolation, and the remaining documented API error classes in `tests/Feature/Api`, `tests/Security/Api`, and `tests/Unit/Api` (SC-004/SC-005) — added: `test_it_serializes_a_real_concurrent_post_race` (real fork-based race against the `postDraft()` row lock, not a mock) and `test_posting_a_vat_line_into_a_settled_period_returns_the_generic_domain_error` (the undocumented-yet-contracted `domain_error` class) in `JournalEntryLifecycleApiTest.php`; `test_exceeding_the_rate_limit_returns_the_documented_429_contract` (a genuine exhausted-limit 429 with `Retry-After`/`X-RateLimit-*`, not just header presence) in `ApiContractTest.php`; four cross-organization isolation tests for invoices and expenses (read + state-changing action) in `BusinessApiSecurityTest.php`. Failed-workflow-retry coverage (idempotency release on 4xx) was already present in `BusinessDocumentApiTest.php` (T058). Full `tests/Feature/Api tests/Security/Api tests/Unit/Api` suite: 114 passed; Pint and PHPStan clean.
