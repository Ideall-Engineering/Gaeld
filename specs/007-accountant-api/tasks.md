...
- [ ] T062 **Not run**: `vite build` fails in this environment with a pre-existing, unrelated native-binding error (`Cannot find module '@rolldown/binding-linux-x64-gnu'`, a known npm optional-dependency bug) — fixing it means removing `node_modules`/`package-lock.json` and reinstalling, which is disruptive enough in this shared environment that it wasn't done without asking first. The Vue changes (Phase 5) were reviewed against existing component patterns but not build-verified or manually smoke-tested in a browser.
...
- [ ] T064 **Not run**: a live pilot smoke in the dev stack's actual running application (not the test suite) would need `PLUGINS_ENABLED=true` turned on there, which wasn't done without asking first — this repo's dev/test containers are shared with a concurrent session. What *is* covered end-to-end by automated tests for all three origins: manual (`JournalCorrectionTest`, web `JournalCorrectionWebTest`), API-sourced (`JournalCorrectionPrepareTest`/`JournalCorrectionLifecycleTest`, `type: 'api'`), and migration-imported (`JournalCorrectionEligibilityTest::test_a_migration_imported_entry_is_eligible`, `type: 'migration'`) — but none of that is the same as a person clicking through the real running app.
